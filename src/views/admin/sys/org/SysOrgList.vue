	...
	<div class="page-wrapper">
		<div class="white-box full-height" v-show="!(editVisible)">
			...
			<div class="search-flex mb16">
				<div class="search-group">
					<el-form label-position="top" :model="searchForm" @keyup.enter.native="refreshList"
						...
						<el-input v-model="searchForm.search" placeholder="编码/名称/拼音码" style="width: 220px"></el-input>
			...
			<el-table ref="table" border :data="tableData" v-loading="tableLoading" row-key="id"
				...
				<el-table-column prop="code" label="组织代码" min-width="150" show-overflow-tooltip></el-table-column>
				<el-table-column prop="type" label="组织类型" min-width="100" show-overflow-tooltip>
          <template v-slot="{row}">
...
<!--				<el-table-column prop="establishmentDate" label="成立日期" min-width="120" show-overflow-tooltip>
          <template v-slot="{row}">
            {{ row.establishmentDate? row.establishmentDate.substring(0, 10) : '' }}
				...
				<el-table-column prop="areaName" label="行政区划名称" min-width="160" show-overflow-tooltip></el-table-column>
