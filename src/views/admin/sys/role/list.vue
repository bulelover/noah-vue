...
<!--            <el-select v-model="searchForm.state" @change="refreshList" style="width: 120px">
      ...
      <el-table ref="table" border :data="tableData" v-loading="tableLoading" row-key="id" :height="g.tableHeight">
        ...
        <el-table-column prop="name" label="角色名称" min-width="120" show-overflow-tooltip>
          <template v-slot="{row}">
            <el-link type="primary" v-if="$hasAuth('sys-role-view')" :underline="false"
        ...
        <el-table-column label="操作" min-width="200" class-name="link-menu" fixed="right">
          <template v-slot="{row}">
            <el-link v-if="$hasAuth('sys-role-menu')" @click="auth(row)" type="primary" :underline="false">角色授权</el-link>
            <el-link v-if="$hasAuth('sys-role-edit')" @click="edit(row)" type="primary" :underline="false">修改</el-link>
            <el-link v-if="$hasAuth('sys-role-delete') && row.id !== 'admin'" @click="remove(row)" type="danger" :underline="false">删除</el-link>
          </template>
        </el-table-column>
      </el-table>
