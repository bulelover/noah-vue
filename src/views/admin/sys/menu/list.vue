    ...
    <div class="white-box full-height">
      ...
      <el-table ref="table" border v-if="refreshTable" :data="tableData" v-loading="tableLoading" lazy row-key="id"
        ...
        <el-table-column prop="name" label="菜单名称" min-width="220" show-overflow-tooltip>
          <template v-slot="{row}">
            <el-link type="primary" v-if="G.hasPerm('sys-menu-view')" :underline="false"
        ...
        <el-table-column prop="state" label="状态" min-width="80" align="center">
          <template v-slot="{row}">
            <template v-if="G.hasPerm('sys-menu-state')">
        ...
        <el-table-column label="操作" min-width="200" class-name="link-menu" fixed="right"
          v-if="G.hasPerm('sys-menu-edit') || G.hasPerm('sys-menu-add') || G.hasPerm('sys-menu-delete')">
          <template v-slot="{row}">
            <el-link v-if="G.hasPerm('sys-menu-edit')" @click="edit(row)" type="primary" :underline="false">修改</el-link>
            <el-link v-if="G.hasPerm('sys-menu-add')" @click="addChild(row)" type="primary" :underline="false">添加下级</el-link>
            <el-link v-if="G.hasPerm('sys-menu-delete')" @click="remove(row)" type="danger" :underline="false">删除</el-link>
          </template>
        </el-table-column>
      </el-table>
    </div>
